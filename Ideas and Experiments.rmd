...
-> does it mean error? or normalization side effect?

## Peak finder
According to Fabio, the code above does not help us, soo now we try to code a new peak finder for only global maxima
...
We substract the value from Ctrl_fraction_max (Ctrl fraction where max) from RNase_fraction_max (RNase fraction where max). If the result:
= 0: no shift
≥ 0: (positive number) left shift  
≤ 0: (negative number) right shift
Only problem: the code does not jet cooperate opsi

```{r}
as.vector(Ctrl_fraction_max) -as.vector(RNase_fraction_max)
```
