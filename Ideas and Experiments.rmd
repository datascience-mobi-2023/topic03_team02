...
## Peak finder
According to Fabio, the code above does not help us, so now we try to code a new peak finder for only global maxima

#### Web search Ctrl peak finder
Here we have a "web search" for protein indices in the Ctrl Dataframe. It shows us in which fraction the maximun protein amount was measured and how much this amount is.
It is nice for visualization or to look up for information quick. But for further analysis, more like a simple vector with just values necessary.
```{r}
...
All rows with "left shift" or "right shift" in the shift_dataframe dataframe are to be compared with all "TRUE" results in the identical_fractions dataframe. If the row names fullfill both criteria in the respective dataframe, I want to create a new dataframe named dogey_proteins, in which these rows have the result "TRUE". 
...
for (row_name in row.names(shift_dataframe)) {
  ...
  if (shift_dataframe[row_name, ] %in% c("left shift", "right shift") & identical_fractions[row_name, ]) {
...
# Print the dogey_proteins dataframe
print(dodgy_proteins)

sum(dodgy_proteins$Result)
```
We have no TRUE results? Mhm... This would mean all our proteins really are true shifters. We'll see...
