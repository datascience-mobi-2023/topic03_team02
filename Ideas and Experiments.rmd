...
```{r}
# Step 1: Select a specific row by name
row_name <- "UTP11_HUMAN" # "ABCA1_HUMAN" (precipitated protein, but still shifter) 'ABT1_HUMAN' (right shifter) #'ABCF1_HUMAN' (left shifter) #UTP11_HUMAN (percipitated non shifting protein?) #ehemals problematisch: SQOR_HUMAN
...
rbp_rows <- y_max_subset[y_max_subset$ProteinType == 'RBP',, drop = FALSE ]

#COMBINE ROWS WITH MAX SHIFT THIS TOOK AGES
#colnames(rbp_rows) <- "Shift"
#rbp_rows$Shift <- ifelse(rbp_rows$Shift == "RBP", "left shift", rbp_rows$Shift)
#max_shifter_names <- rbind(max_shifter_names, rbp_rows)
...
#rbp_rows_names <- rownames(rbp_rows)
