...
print(rows_with_multiple_peaks)

```
```{r}
# Step 1: Extract the row you want to find peaks in
row_index <- 3  # Adjust the row index as needed
row_data <- Mitosis_Ctrl_mean[row_index, ]
...
Option of a more efficient code from ChatGTP
```{r}
# Step 1: Find the indices of local maxima in a vector
find_local_maxima <- function(x) {
  is_local_max <- c(FALSE, x[-1] > x[-length(x)], FALSE)
  local_maxima_indices <- which(is_local_max)
  return(local_maxima_indices)
}

# Step 2: Iterate over all rows in the dataframe and identify rows with multiple peaks
rows_with_multiple_peaks <- which(
  apply(Mitosis_Ctrl_mean, 1, function(row) {
    local_maxima_indices <- find_local_maxima(row)
    length(local_maxima_indices) > 1
  })
)

# Step 3: Print the row indices with multiple peaks
print(rows_with_multiple_peaks)
