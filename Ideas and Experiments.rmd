...
for (row_index in 1:nrow(Mitosis_Ctrl_mean)) {
  ...
  # Step 4: Check if the row has multiple peaks
  if (length(peaks) > 7) {  #More than 7 peaks!
    rows_with_multiple_peaks <- c(rows_with_multiple_peaks, row_index)
  }
}
...
#y-shifts
results_yshift #Ist glaub noch nicht fertig @Jimena
```
...
```{r}
# Step 1: Select a specific row by name
row_name <- "SQOR_HUMAN" #'ABT1_HUMAN' (right shifter) #'ABCF1_HUMAN' (left shifter)
...
```{r}
Ctrl_maxima["SQOR_HUMAN",]
RNase_maxima["SQOR_HUMAN",]
```
