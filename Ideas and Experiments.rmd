...
```{r}
# Step 1: Select a specific row by name
row_name <- "ABCB7_HUMAN" # "ABCA1_HUMAN" (precipitated protein, but still shifter) 'ABT1_HUMAN' (right shifter) #'ABCF1_HUMAN' (left shifter) #UTP11_HUMAN (percipitated non shifting protein?) #ehemals problematisch: SQOR_HUMAN #RBM3_HUMAN complicated but cool
...
# Step 4: Visualize the peaks on a plot
plot(1:25, Mitosis_Ctrl_100[row_name, ], type = "l", col="blue", xlab = "Fractions", ylab = "Relative Protein amount",ylim=c(0,30), main = row_name)
points(peaks_Ctrl, row_data_Ctrl[peaks_Ctrl], col = "blue", pch = 16)
...
venn_result <- venn.diagram(
  ...
  fill = c("dodgerblue", "darkorange", "springgreen"),  # Specify colors for the diagram elements
...
venn_result <- venn.diagram(
  ...
  category.names = c("Max Shifters", "Local Shifters", "Curve Shifters", "RDeep shifters"),
  filename = NULL,  # Set filename to NULL to prevent saving as an image file
  fill = c("dodgerblue", "darkorange", "springgreen", "red"),  # Specify colors for the diagram elements
  alpha = 0.5,  # Set transparency for the overlapping areas
  cex = 1.5  # Increase the font size
)
...
# Extract the row names from each dataframe
curvez_shifter_row_names <- rownames(curvez_shifter_names)
RBP_row_names <- rownames(comparison_RBP)

# Create the Venn diagram
venn_result <- venn.diagram(
  x = list(curvez_shifter_row_names, RBP_row_names),
  category.names = c("Curvez Shifters", "RDeep shifters"),
  filename = NULL,  # Set filename to NULL to prevent saving as an image file
  fill = c("dodgerblue", "red"),  # Specify colors for the diagram elements
  alpha = 0.5,  # Set transparency for the overlapping areas
  cex = 1.5  # Increase the font size
)
