...
```{r}
# Step 1: Select a specific row by name
row_name <- "SQOR_HUMAN" #'ABT1_HUMAN' (right shifter) #'ABCF1_HUMAN' (left shifter) #UTP11_HUMAN PERCIPITATED PROTEIN??
...
# Step 4: Visualize the peaks on a plot
plot(1:25, Mitosis_Ctrl_100[row_name, ], type = "l", col="blue", xlab = "Fractions", ylab = "Relative Protein amount",ylim=c(0,40), main = row_name)
points(peaks_Ctrl, row_data_Ctrl[peaks_Ctrl], col = "blue", pch = 16)
...
quotients_minmax #Werte umdie 1, je mehr sie abweichen, desto eher sind es shifter 
#-> usable
...
comparison_RBP <- read.table("https://github.com/datascience-mobi-2023/topic03_team02/blob/5da6f789d96f1924a96e731de447dbf35b41c728/Datensatz_RBP2GO_Proteins.txt", header = TRUE)

# View the loaded data
head(comparison_RBP)
```

#### 4.2 Max shift mit RBP2go-Vergleich

```{r Max shift Vergleich}

# 1. max shift Daten Einteilen in RBP und non-RBP
...
#### 4.3 Local max shift mit RBP2go-Vergleich

#### 4.4 Curve quotients mit RBP2go-Vergleich

#### 4.5 Y-shift mit mit RBP2go-Vergleich
