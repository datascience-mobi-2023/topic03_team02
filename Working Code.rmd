...
This code only identifies the number of local shifts and is only used for kmeans and linear regression later.

Here, the only thing which can be rightly quantified and thus considered, is how many local maxima appear or which disappear between control and RNase. The question whether the fractions of these peaks are identical cannot be quantified directly and thus is left out. As a result, quite a few proteins are neglected, more precisely 670 proteins are not listed (proteins with same amount of peaks but which are found in different fractions).
