...
For our shift proteins, we now want to compare all the location of the absolute and local maxima in Control and RNase with each other. Here, I save the column indices of Mitosis_RNase_100, with all maxima, no matter local or absolute, in a new dataframe (RNase_maxima) and repeat this process with Mitosis_Ctrl_100. 

Then I compare if the number of the maxima AND the column indices of each row are identical. If BOTH is identical, then I would like the result TRUE to be saved in for this row specifically. If not, then false. 

TRUE: no shift, da alle local maxima identisch
FALSE: shifter
```{r}
...
sum(shift_dataframe$Shift == "left shift") + sum(shift_dataframe$Shift == "right shift")
```
Nice! Both methods seem to give the EXACT SAME PROTEINS?? Or there is a issue with the code which I do not entirely comprehend yet. (Only comparing the number would not guarantee that the proteins are identical, but I try to avoid this using this code.)
...
Eine Möglichkeit jetzt wäre zu sagen, wir gucken welche proteine im Quotienten einen Wert tiefer als 0,8 ist, also eine abweichung um 25% hat (z.B. : Ctrl=4, und RNase=4 * 1,25, also 25% größer, und 4/(4 * 1,25) =0,8). Oder eben einen Wert größer als 1,25. Dann würden wir diese Proteine als shifter definieren.
...
```
This is A LOT OF SHIFTERS... Maybe we still have to adjust the threshold.

Alternative method to determine shifters via statistcal test (t test doesnt work bc in some rows the values do not vary enough).

```{r Wilcoxon signed rank test}
# Create an empty vector to store the p-values
p_values <- vector("numeric", nrow(curve_quotients))

# Perform Wilcoxon signed rank test on each row
for (i in 1:nrow(curve_quotients)) {
  row_values <- as.numeric(curve_quotients[i, ])
  if (any(is.na(row_values))) {
    p_values[i] <- NA
  } else {
    p_values[i] <- wilcox.test(row_values, mu = 1, alternative = "two.sided", exact = FALSE, na.rm = TRUE)$p.value
  }
}

# Create the curve_p_values dataframe
curve_p_values <- data.frame(
  Protein = rownames(curve_quotients),
  p_value = p_values,
  stringsAsFactors = FALSE
)

# Add a column for Curve_shifters based on p-value significance level
curve_p_values$Curve_shifters <- ifelse(curve_p_values$p_value < 0.05, "shifter", "no shifter")

# Print the curve_p_values dataframe
print(curve_p_values)

```
This does not make sense yet! I am working on it though :)
