...
#This step was repeated later again, see code line 291, after we have split the dataset (which is after data cleanup, but the argumentation applies just the same. If rows before were still are all 0, they are still 0 afterwards, which is definitely an experimental error)

dim(RDeeP_HeLa_Mitosis)
```

#### 1.3 Checking for nonzero rows and negative values.
...
nrow(nonzero_rows) #320 rows do not have zeros. But we decided this as not relevant for our further analysis
...
rm(nonzero_rows, column_values,max_values_Ctrl,max_values_RNase,p_values_Ctrl,p_values_RNase,RDeeP_HeLa_Mitosis_p,bad_proteins10,bad_proteins10wierd,num_sets,p_values,protein1_Ctrl_frac1,protein8_Ctrl_frac1,bad_proteins5,i,RDeeP_HeLa_Mitosis_Ctrl,RDeeP_HeLa_Mitosis_RNase)
```
...
row_name <- "2A5E_HUMAN" # "ABCA1_HUMAN" (precipitated protein, but still shifter) ABT1_HUMAN (right shifter) #ABCF1_HUMAN and RBM3_HUMAN (left shifter) #UTP11_HUMAN (percipitated non shifting protein?) #ehemals problematisch: SQOR_HUMAN
...
#### 4.1 Max shift: Global Peak finder
...
'=','>-1'and'<1' 0: no shift
'>1': left shift (max fraction in Control larger than max fraction in RNase)
'<-1': right shift (max fraction in Control larger than max fraction in RNase)
...
```
##### 4.1.1  y-shift for partial shifter

After the max shift, we execute a y shift on all proteins which had 0, 1 or -1 shifts in the first part (no significant max shift). This allows us to identify additional partial shifter.
...
##### 4.2.1 Local Maxima code and linear regression 

This code quantifies the local shifts and is only used for kmeans and linear regression later. In order to do this I select all proteins which do not have any differences in the local peaks, and then assign the value 1 for all proteins in this subcategory which have different column indexes in their peaks. If the peak number AND all column indexes are identical, the proteins have the value 0. If we only take the difference in numbers of overall peaks, quite a few proteins are neglected, more precisely 670 proteins are not listed (proteins with same amount of peaks but which are found in different fractions). Using this method, we can quantify the local shift for later evaluation in the linear regression. 
...
Now we look at which proteins in the quotient have a value lower than 0.8, i.e. a deviation of 25% (e.g. : proteins with values greater than 1.25 would be assigned as shifters.) -> a lot of proteins are identified, possibly falsly including several statistcal fluctuations and identifying them as shifters
--> 70% is a much more reliable margin.
...
##### 6.1.1 Trying out new dataframes (caution: same name!!!!!!!!!)
Okay, mit der ersten Methode haben wir jetzt einen R Wert von 0.2658505. Das ist nicht wirklich gut genug. Im folgenden 2 dataframe, die dann jeweils auch shift_comparison heißen. Dementsprechend immer den chunk des DF vor den Finden der beiden columns mit der höchsten linearen Beziehung ausführen.
...
###6.4 QQ-Plot of the linear regression
