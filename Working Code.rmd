...
Finding the maximum value for each row, then deciding if it is smaller than our significance level (either 5% or 1%, has to be decided beforehand)
...
We still have to determine which p-values are over 5% and use that information on the dataframe. But now first all the same for the RNase:
...
#Testing how many RNase values I have above 5%/10%:
...
Now adding the columns to the main dataframe and doing the same, identify the rows in which the p-value exceeds 5%:
```{r}
RDeeP_HeLa_Mitosis_p <- data.frame(RDeeP_HeLa_Mitosis, max_values_Ctrl, max_values_RNase) 

# Identify all rows where the value in _one_ of the last two columns exceeds 5/10%
...
#Alternative 5%/10% BUT CAUTION! This sorts only the rows in which BOTH Control AND RNase exceed 5%? MAYBE? BUT THERE ARE MORE VALUES? HUHHH WAIT IM CONFUSED 
#bad_proteins5 <- which(RDeeP_HeLa_Mitosis_p[, 151:152] > 0.05)
#bad_proteins10 <- which(RDeeP_HeLa_Mitosis_p[, 151:152] > 0.1)

length(bad_proteins5)
length(bad_proteins10)
```
Thats more than half (or more than a quarter) of our data...

Anywayyyy, let's work with the 10 % margin and delete all corresponding rows.

```{r}

RDeeP_HeLa_Mitosis_clean <- RDeeP_HeLa_Mitosis_p[!(RDeeP_HeLa_Mitosis_p[, 151] > 0.05 | RDeeP_HeLa_Mitosis_p[, 152] > 0.05), ] #the ! Operator negate the condition.It excludes rows where the condition is TRUE, thus effectively filtering out rows that exceed 10%.
...
```{r}
#First finding the mean of all three replicates for every fraction separately in our RDeep_Hela_Mitosis_Ctrl data:
