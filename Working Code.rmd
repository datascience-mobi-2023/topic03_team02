...
```
#### 4.2.1 logischer y-shift

nach max shift den Y shift durchführen mit alen proteinen die 0 als max shift hatten und somit weitere partial shifter identifizieren.

```{r}
max_shift <- Ctrl_fraction_max - RNase_fraction_max

# Assuming 'max_shift' is your data frame with the 'Fraction' column
max_shift_0 <- max_shift[max_shift$Fraction %in% c(0, 1, -1), , drop = FALSE]
...
We get TRUE results for all NO SHIFT proteins, meaning they have identical local and absolute maxima. 

#### 4.2.1 Local Maxima code for kmean und linear regression 

Dieser Code identifiziert nur die Anzahl der local shifts und wurde nur für das kmeans und linear regression gebracht
...
results_yshift <- data.frame(row_global_max_Ctrl, row_global_max_RNase)
show(results_yshift) 
...
```
Sagt mir halt irgendwie nichts
...
#### 6.4 Trying out new dataframes (caution: same name!!!!!!!!!)
